...
| M1 Wasm 后端 | **进行中**（PR #10–13 已完成：wit 世界 v1 + 工具链定型 + 宿主加载/驱动 + WasmComponent 接入 cordis-core + wasm 依赖者消费 + **双后端共存**（同一 loader 加载原生与 wasm，值类型统一互通）——**门禁 1/3 达成**） | 同一 loader 加载原生与 wasm 组件；guest 崩溃不伤宿主；Rust + Go 双语言 guest；走查 §6.2–6.4 |
| M2 加载器 + HMR | 未开始 | — |
| M3 案例验证 | 未开始 | — |
