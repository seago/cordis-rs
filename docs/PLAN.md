...
| M2 加载器 + HMR | **进行中**（PR #17：L-Raise 失败模型落地——处置⑤/⑧ 收官：`FiberError::raise` + reload 捕获 → 失败 outcome + ζ 析取；wasm 桥接统一 trap/越界 set/绑定冲突为失败模型；`failure_model.rs` + `sandbox_isolation.rs` 升级直证） | 改插件代码保存即生效，in-flight 任务不中断、其他组件状态保留；回滚用例；走查 §5.2 |
| M3 案例验证 | 未开始 | — |
