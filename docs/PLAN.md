...
| M1 Wasm 后端 | **进行中**（PR #10 已完成：wit 世界 v1 + 工具链定型（wasm32-wasip2 + no_std guest + wasmtime 47）+ 宿主加载/驱动原语 + Rust guest 示例端到端闭环） | 同一 loader 加载原生与 wasm 组件；guest 崩溃不伤宿主；Rust + Go 双语言 guest；走查 §6.2–6.4 |
| M2 加载器 + HMR | 未开始 | — |
| M3 案例验证 | 未开始 | — |
