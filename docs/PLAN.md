...
| M3 案例验证 | **完成（走查 §5.3 通过，门禁判定 5/5）**：PR #24 im-bot 三层拓扑 + §6.2 broker（处置⑦ 落地、REVIEW-d1263fa 闭环）；PR #25 bench 报告（notify 扇出 + 切换延迟，三层分离测量、REVIEW-bbb252a 闭环）；PR #26 处置⑩⑪⑫ 评估收尾（REVIEW-d457b60 闭环）；PR #27 走查 §5.3（无未解释偏差，2 项范围说明非偏差）——案例断言 / bench 产出 / 走查三门禁全达成 | adapter/database/功能插件三层依赖拓扑案例；bench 报告（notify 扇出、切换延迟）；走查 §5.3 |
