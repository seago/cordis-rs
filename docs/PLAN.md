...
| M2 加载器 + HMR | **进行中（主目标 + 处置全落地）**（PR #17-22：L-Raise 失败模型、interception 求值形态、loader 全字段 + group/include + Algorithm 7、cordis-hmr（保存即生效 + 双回滚直证）、Alg 6 Proxy 访问层、处置⑥⑨ 评估收尾——**剩余：走查 §5.2 + 门禁判定（PR #23）**） | 改插件代码保存即生效，in-flight 任务不中断、其他组件状态保留；回滚用例；走查 §5.2 |
| M3 案例验证 | 未开始 | — |
