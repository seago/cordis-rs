...
| M1 Wasm 后端 | **进行中**（PR #10–14 已完成：wit 世界 v1 + 工具链定型 + 宿主加载/驱动 + WasmComponent 接入 cordis-core + wasm 依赖者消费 + 双后端共存（值类型统一互通）+ **沙箱隔离**（恶意 guest trap 被捕获、宿主存活）+ **Rust + Go 双语言 guest**（标准 go + wasip1 + 预览1 适配器组件化）——**门禁 1/3、2/3、3/3 全部达成**，待走查 §6.2–6.4） | 同一 loader 加载原生与 wasm 组件 ✓；guest 崩溃不伤宿主 ✓；Rust + Go 双语言 guest ✓；走查 §6.2–6.4（PR #15，进行中） |
| M2 加载器 + HMR | 未开始 | — |
| M3 案例验证 | 未开始 | — |
