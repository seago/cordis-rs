...
| M2 加载器 + HMR | **进行中**（PR #17：L-Raise 失败模型落地——处置⑤/⑧ 收官；PR #18：interception 求值形态——处置① 元数据侧落地（get_meta/declared_metadata/intercept_in_place）） | 改插件代码保存即生效，in-flight 任务不中断、其他组件状态保留；回滚用例；走查 §5.2 |
| M3 案例验证 | 未开始 | — |
