...
| M3 案例验证 | **进行中**（PR #24 已闭环：IM-bot 三层拓扑案例 + broker 示例（处置⑦ 落地，审查重排依赖方向后全断言）——adapter/database/功能插件拓扑全断言：切换后端/重连/依赖不可用；broker：更新/卸载后备不扰动 broker 与消费者、可逆注册自动撤销、重注册自动恢复；PR #25 基准报告（notify 扇出 + 切换延迟，三层分离测量 + 审查修复闭环）完成；剩余：处置⑩⑪⑫ + 走查 §5.3） | adapter/database/功能插件三层依赖拓扑案例；bench 报告（notify 扇出、切换延迟）；走查 §5.3 |
