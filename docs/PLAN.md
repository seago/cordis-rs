...
| M2 加载器 + HMR | **进行中（首批任务：失败模型/拦截/加载器前置）**（PR #17：L-Raise 失败模型——处置⑤/⑧ 收官；PR #18：interception 求值形态——处置① 元数据侧；PR #19：loader 全字段 + group/include 配置树；PR #20：托管 realm + Algorithm 7 重指派——**HMR 主目标未开始**） | 改插件代码保存即生效，in-flight 任务不中断、其他组件状态保留；回滚用例；走查 §5.2 |
| M3 案例验证 | 未开始 | — |
