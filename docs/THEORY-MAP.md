...
| 2026-08-22 / P-3 | **core 授权（产品验证线 P-3，REVIEW-PHASE2-PROPOSAL nit-2 单独立案）**：`Runtime.suspended` 挂起集（登记：激活挂起分支/advance 再挂起；撤销：advance 完成/unload 收账）+ `suspended_fibers()` 查询 + `advance_suspended(judge)` 批量恢复——Await 生产化（B 计划后续，宿主可枚举/上报/批量驱动挂起 fiber）；额度=本线范围不扩面 | §4.3.3 异步 | 扩展（授权） | 记录 |
