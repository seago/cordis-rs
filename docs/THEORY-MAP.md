...
| 2026-08-17 / PR #25 | **M3-PR2：基准报告（§5.1.2 notify 扇出 + §5.3 切换延迟）**——`examples/im-bot/src/bin/bench.rs`（std::time 手动基准，零依赖，reps=5 中位数 + fresh loader 防 no-op）：场景 A 扇出（1 提供者 + N∈{1,10,100,1000} 消费者注入同一键——激活/停用/再激活耗时随 N 扩展，每消费者常数成本、超线性残差归因 Algorithm 3 O(F) 逐 fiber 扫描）；场景 B 切换延迟（三层 + M∈{0,100,500} 无关组件——电机重激活局部性：bot 效应恰重执行 1 次、adapter 0 次、fiber 不变、填充组件全程 Active，与 M 无关；切换延迟 O(F) 扫描主导）。报告 `docs/bench/M3-BENCH.md`（release 数据表 + 解读 + 对 §5.3 "future work" 量化测量的补充 + 已知边界：realm 依赖倒排索引）。CI 门禁：bench bin 运行（宽松绝对上界，debug 余量 >60×） | §5.1.2, §5.3 | 完成（PR #25：基准报告 + 门禁） | 记录 |
