...
| 2026-08-22 / backlog ② | **core 授权（backlog 清理线②，判据 v2 启用）**：`Step::Await` 增判据载荷 `Await(Option<Box<dyn Fn() -> bool>>)`（挂起时自报就绪判据，随挂起上下文存亡——advance 完成/unload 收账即释放）+ `Runtime::poll_ready()`（组合线程**显式驱动点**统一评估自报判据 → 满足即 advance——"谁在何时 poll" = 组合线程显式点、无自主轮询/timer，ADR-0002 单线程 push 保持）；`Await(None)` 保持外部判据 + `advance_suspended` 语义（P-3 评估结论兼容，不启用自动轮询）；wasm 桥 `poll_and_advance` 接线（每 fiber 在途 rep 追踪，未就绪保持挂起不空转——相对 `advance_suspended(\|_\| true)` 全驱的精确化）。添加性：既有迭代器不产 Await → 执行语义零变化；try_execute_with Err 三元组（判据透传）为内部形态扩展。授权：用户 2026-08-22 backlog 决策 ②-1/②-2（docs/cordis-BACKLOG-123-PLAN.md） | §4.3.3 异步, Def 51 | 扩展（产品层，授权） | 记录 |
