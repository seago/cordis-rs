...
| 2026-08-16 / PR #13 审查 | **记录（m2，REVIEW-1df64a1）**：`Value` 类型住在 cordis-wasm——原生组件与 wasm 互通须依赖 wasm crate（仅为一个值类型），依赖方向"原生 → wasm"与既有分层不一致；边界已记录（crate 文档 + THEORY-MAP），生产化时下沉 `Value` 到 core/独立 value crate（M2 或正式双后端支持前处理） | ADR-0004 | 记录（架构边界，M2 处置） | 记录 |
...
| 2026-08-17 / PR #24 | **M3-PR1：IM-bot 迷你案例（§5.3 Koishi 式三层拓扑）+ 处置⑦ broker 示例**——`examples/im-bot`：adapter 层（提供 platform）/ database 层（提供 db）/ 功能插件 bot（注入两层、提供 reply）；运行时操作直证——切换存储后端（同供给键替换）只重激活解析依赖变化的依赖者（bot 重激活、fiber 不变）而 adapter 不受影响；重连 adapter（退役→移除→重装）→ bot 级联停用再自动重连、database 不受影响；依赖不可用 → bot 保持 Inactive 不报错、adapter 出现自动激活（§5.3 "stays inactive until it appears, without erroring"）；broker 示例（§6.2）：更新后备不扰动消费者（无 reload）、卸载后备 = 可逆注册自动撤销、重注册自动恢复。CI 门禁：im-bot + broker 双 bin 运行 | §5.3, §6.2 | 完成（PR #24：案例验证起步） | 记录 |
...
| ⑦ | §6.2 broker 示例（可表达性演示） | 本次新增 | **已落地（M3-PR1，PR #24）**：`im-bot` broker 示例——两后备提供者（不同注册键）+ broker（注入注册键、提供 service）+ 消费者；更新后备不扰动消费者（无 reload）；卸载后备 = 可逆注册自动撤销（broker 停用、service 解除）；重注册自动恢复 |
