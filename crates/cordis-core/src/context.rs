...
impl Context {
    ...
    /// REVIEW-2a7a686 m1/m2）——内部经 `resolve_realm` 解析目标 realm
    /// （隔离语义由核心承担），Def 43/48 纪律按**键**判定（`provide`
    /// 声明的是键符号）；notify、逆 = 撤销绑定与 typed 版本一致，仅
    /// 值类型擦除。
    ///
    ...
    pub fn set_dyn(
        ...
        let realm = self.resolve_realm(key);
        // Def 43/48 纪律：组件只写自己声明的供给（按键判定，同 `set`）。
        if let Some(fid) = self.fiber {
            let allowed = self
                .runtime
                .fibers
