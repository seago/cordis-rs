...
/// C2 端到端：两阶段 guest——阶段 1 提交 → 宿主回填 → 回注（preseed 镜像）
/// → revision bump 阶段 2 读回继续（probe_out），失败 path 兜（probe_err）。
#[test]
fn c2_two_phase_guest_consumes_backfilled_result() {
    let worker = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .build()
        .expect("worker runtime");
    ...
    let runtime = Rc::new(Runtime::new());
    let loader = Rc::new(cordis_loader::Loader::new(Rc::clone(&runtime)));
    loader.register_component("db", Rc::clone(&comp) as Rc<dyn Component>);

    // 阶段 1：激活（提交 echo(7)）→ worker 回填 Count(14)。
    loader.apply(&[cordis_loader::Entry::new("p", "db", Rc::new(()), 0, false)]);
    assert!(comp.bindings().contains_key("db"), "阶段1 绑定 db");
    let result = await_remote_value(&comp, 0);
    assert!(matches!(result, Value::Count(14)), "回填 echo(7)→14");

    // 回注（C 探针：宿主向镜像预注输入，供阶段 2 消费）。
    comp.preseed_mirror("probe_in", result);

    // 阶段 2：revision bump 重建 → guest 经镜像读 probe_in 继续。
    loader.apply(&[cordis_loader::Entry::new("p", "db", Rc::new(()), 1, false)]);
    let out = runtime
        .store()
        .get_value(cordis_core::symbol::Symbol::intern("probe_out"))
        .and_then(|v| v.downcast_ref::<Value>())
        .cloned();
    assert!(
        matches!(out, Some(Value::Count(15))),
        "阶段2 消费回注结果（14+1=15）：{out:?}"
    );

    // 失败路径：回注为不可处理类型（Text 非 Count）→ 阶段 2 兜 probe_err。
    comp.preseed_mirror("probe_in", Value::Text("boom".into()));
    loader.apply(&[cordis_loader::Entry::new("p", "db", Rc::new(()), 2, false)]);
    let err = runtime
        .store()
        .get_value(cordis_core::symbol::Symbol::intern("probe_err"))
        .and_then(|v| v.downcast_ref::<Value>())
        .cloned();
    assert!(
        matches!(err, Some(Value::Text(ref t)) if t == "boom"),
        "失败 path 经回注传播（阶段2 兜 probe_err）：{err:?}"
    );
